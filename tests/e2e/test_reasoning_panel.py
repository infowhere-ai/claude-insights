...
def test_thinking_block_appears(page: Page, server: ServerContext, project: str) -> None:
    ...
    """
    server.write_jsonl(
        project, [server.assistant_entry(thinking="I need to carefully analyse this problem.")]
    )

    page.goto(f"{server.url}/insights")
    page.locator("#project-select").select_option(label=project)

    expect(page.locator("#thinking-text")).to_contain_text("carefully analyse", timeout=TIMEOUT)
...
def test_empty_thinking_block_not_shown(page: Page, server: ServerContext, project: str) -> None:
    ...
    """
    server.write_jsonl(
        project, [server.assistant_entry(thinking="   \n\n  ")], filename="session-no-think.jsonl"
    )
    ...
    expect(page.locator("#thinking-text")).not_to_contain_text("carefully analyse", timeout=2000)
...
def test_thinking_history_modal_opens(page: Page, server: ServerContext, project: str) -> None:
    ...
    server.write_jsonl(
        ...
        [
            ...
            server.assistant_entry(thinking="Second deeper analysis.", ts="2026-01-01T10:00:01Z"),
        ],
    )
